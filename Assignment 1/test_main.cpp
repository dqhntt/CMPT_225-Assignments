...
TEST_CASE("Full tests") {
    ...
    SUBCASE("Insert") {
        ...
        SUBCASE("Get and remove") {
            ...
            CHECK(a.get(4).getName() == a.remove(4).getName());
            CHECK_THROWS(a.get(5));
            CHECK_THROWS(a.remove(5));
            CHECK_THROWS(a.get(4));
            CHECK_THROWS(a.remove(4));
            CHECK(a.get(2).getArtist() == a.remove(2).getArtist()); // a = 12 14 11
    ...
    SUBCASE("Swap") {
        ...
        SUBCASE("Container swap") {
            REQUIRE_NOTHROW(swap(c, a));
            CHECK(a == c);
            REQUIRE_NOTHROW(swap(c, a));
            CHECK(c == a);
            REQUIRE_NOTHROW(swap(a, c));
            CHECK(a == c);
            REQUIRE_NOTHROW(swap(a, a));
            CHECK(a == a);
            CHECK(c == a);
        }
    }
...
TEST_CASE("Stress tests") {
    ...
    SUBCASE("Insert at front") { // INT_MAX ... 1
        for (int i = 0; i < INT_MAX; i++) {
            REQUIRE_NOTHROW(rev.insert(Song("s", "t", i + 1), 0));
            REQUIRE(rev.get(i).getName() == "s");
            REQUIRE(rev.get(i).getArtist() == "t");
            REQUIRE(rev.get(i).getLength() == i + 1u);
            REQUIRE(rev.size() == i);
        }
    }
    ...
    SUBCASE("Swap") {
        SUBCASE("Reverse order") { // 1 ... size
            ...
            for (unsigned i = 0; i < half; i++) {
                WARN_NOTHROW(ord.swap(i, size - i - 1u));
            }
            for (unsigned i = 0; i < size; i++) {
                REQUIRE(ord.get(i).getLength() == i + 1u);
            }
        }
